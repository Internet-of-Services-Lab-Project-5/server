...
import { Passenger, TaskStatus } from "../types.ts";
...
const reportURL = ""; //TODO: get from .env after deploy
...
export class GramineClient {
  ...
  async order(passengers: Passenger[]) {
    try {
      ...
      const urlKey = await this.pushSecret(process.env.STAGING_URL || "");
      const requestorder = await this.getRequestOrder(
        apporder,
        workerpoolorder,
        [argKey, urlKey]
      );
  ...
  private async getAppOrder() {
    ...
    const appOrder = await this.iexec.order.signApporder(
      appOrderToSign,
      orderOptions
    );
    return appOrder;
...
export class SconeClient {
  ...
  async updateDataset() {
    ...
    try {
      ...
      await this.iexec.dataset.pushDatasetSecret(
        address,
        encryptionKey,
        sconeSceretOptions
      );
      ...
      const datasetOrder = await this.iexec.order.signDatasetorder(
        datasetOrderToSign,
        orderOptions
      );
      const orderHash = await this.iexec.order.publishDatasetorder(
        datasetOrder
      );
  ...
  private async encryptDataset() {
    ...
    const encryptedDataset = await this.iexec.dataset.encrypt(
      datasetFile,
      encryptionKey
    );
    const checksum = await this.iexec.dataset.computeEncryptedFileChecksum(
      encryptedDataset
    );
