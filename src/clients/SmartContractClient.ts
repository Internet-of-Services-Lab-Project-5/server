...
export class SmartContractClient {
    private static instance: SmartContractClient;

    private contract?: Contract;
    constructor() {}

    static getInstance(): SmartContractClient {
        if (!SmartContractClient.instance) {
            SmartContractClient.instance = new SmartContractClient();
            SmartContractClient.instance.init();
        }
    ...
    async proposeAirline(name: string, ethAddress: string, iExecAddress: string) {
        ...
        console.log("contract", this.contract);
        const propose: (name: string, ethAddress: string, iExecAddress: string) => Promise<any> = this.contract.getFunction("propose");
    ...
    onAdded(callback: (data?: any) => void) {
        if (!this.contract) {
            console.log("contract not initiated yet");
            return;
        }
        this.contract.on("AirlineAdded", callback);
        console.log("listening");
    }

    onRejected(callback: (data?: any) => void) {
        if (!this.contract) {
            console.log("contract not initiated yet");
            return;
        }
        this.contract.on("AirlineRejected", callback);
        console.log("listening");
    }
}
